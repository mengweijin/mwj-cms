...
public class IdUtils extends IdUtil {
	...
	public static String timestampID() {
		synchronized(TIMESTAMP_ID){
			// %d：格式化整型数字，0：数字前面补0，3：总共长度为3
			String number = String.format("%03d", new Random().nextInt(1000));
			return DateFormatUtil.format(new Date(), DateFormatUtil.YYYYMMDDHHMMSSSSS) + number;
		}
