...
# 线程调度器实例名 默认：DefaultQuartzScheduler  属性可为任何值，用在 JDBC JobStore 中来唯一标识实例，但是所有集群节点中必须相同。
...
# 单位毫秒，执行时间落后当前时间60秒内判定为错过执行时间，然后会根据设定的misfire策略进行处理，落后大于60秒就不作补救措施了
org.quartz.jobStore.misfireThreshold: 60000
############################################################################################################
## 保存quartz quartzJobParams 在内存中的配置   使用内存存储任务和触发器等信息
## org.quartz.jobStore.class: org.quartz.simpl.RAMJobStore
## 持久化quartz quartzJobParams 到数据库的配置   使用数据库存储任务和触发器等信息
## 属性为 JobStoreTX，将任务持久化到数据中。因为集群中节点依赖于数据库来传播 Scheduler 实例的状态，
## 你只能在使用 JDBC JobStore 时应用 Quartz 集群。这意味着你必须使用 JobStoreTX 或是 JobStoreCMT 作为 Job 存储；
## 你不能在集群中使用 RAMJobStore。
#############################################################################################################
org.quartz.jobStore.class = org.quartz.impl.jdbcjobstore.JobStoreTX
# 驱动代理，需要根据不同数据库来选择不同的类
org.quartz.jobStore.driverDelegateClass = org.quartz.impl.jdbcjobstore.StdJDBCDelegate
# org.quartz.jobStore.driverDelegateClass = org.quartz.impl.jdbcjobstore.oracle.OracleDelegate
# quartz数据库表默认的前缀
org.quartz.jobStore.tablePrefix = QRTZ_
# JobDataMaps是否都为String类型， 个人测试结果：加入下面的配置依然报类型转换错误，所以还是传入json字符串好
# org.quartz.jobStore.useProperties = false

#############################################################################################################
## 是否设置需要集群 集群要求必须使用持久化存储， 如果只有一个节点，不影响。
## 属性为 true，你就告诉了 Scheduler 实例要它参与到一个集群当中。这一属性会贯穿于调度框架的始终，用于修改集群环境中操作的默认行为。
#############################################################################################################
org.quartz.jobStore.isClustered = true
# 集群节点的ID则由quartz自动生成，属性为 AUTO即可，基于主机名和时间戳来产生实例 ID。
org.quartz.scheduler.instanceId = AUTO

#############################################################################################################
## org.quartz.jobStore.clusterCheckinInterval 属性定义了Scheduler 实例检入到数据库中的频率(单位：毫秒)。
## Scheduler 检查是否其他的实例到了它们应当检入的时候未检入；这能指出一个失败的 Scheduler 实例，
...
# quartz使用应用的数据源时，无需下面配置数据源
#org.quartz.jobStore.dataSource = myDS
#org.quartz.dataSource.myDS.driver = com.mysql.jdbc.Driver
#org.quartz.dataSource.myDS.URL = jdbc:mysql://localhost:3306/dreamne?characterEncoding=utf-8
